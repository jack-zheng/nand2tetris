...
花费三个半月的业余时间，终于将这个项目完结了，总的来说很棒的一个项目，如果国内相关专业都拿这个当 计组 的教材就好了。很有趣也很有挑战。就学习资料来说，视频 > 课件 > 书本。书本本身更像是一个目录，视频是主体，课件对完成练习很有帮助, 在课程网站有下载。课程网站还有一个问答板块，有问题可以去搜一下(全英文，要翻墙)。
...
memory 芯片设计的时候有一个很巧妙的地方，地址线的 13 位在 ram16k 和 sel 之间是公用的。一开始想不通，但是将可能的情况列举了一下发现，很巧妙。总的来说 dmux 中，它用 0x 左右 ram 的片选信号，x 是13 位，0，1 都无所谓。1x 作为 screen 和 board 的片选。因为 screen 只有 8k 用不到 13 号线，所以没有关系，妙啊。

组 CPU 可太难了，需要重新巩固一下之前各个元器件的几本定义，输入输出才可能完成，抄了一下答案，算是过了。不过大致有了一个 sense，看看第六章再决定要不要回头再看。
